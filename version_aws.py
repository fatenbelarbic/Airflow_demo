...
download_mnist_data = S3CopyObjectOperator(
    ...
    dest_bucket_name="your-bucket-aawse-eu-west-1",
...
def _extract_mnist_data():
    ...
    mnist_obj = s3hook.get_key(bucket_name="your-bucket-aawse-eu-west-1", key="mnist.pkl.gz")
    ...
    with gzip.GzipFile(fileobj=mnist_buffer, mode="rb") as f:
        ...
        s3hook.load_file_obj(
            output_buffer, key="mnist_data", bucket_name="your-bucket-aawse-eu-west-1", replace=True
        )
...
sagemaker_train_model = SageMakerTrainingOperator(
    ...
    config={
        ...
        "InputDataConfig": [
            {
                ...
                "DataSource": {
                    "S3DataSource": {
                        "S3DataType": "S3Prefix",
                        "S3Uri": "s3://your-bucket-aawse-eu-west-1/mnist_data",
                        "S3DataDistributionType": "FullyReplicated",
                    }
                },
            }
        ],
        "OutputDataConfig": {"S3OutputPath": "s3://your-bucket-aawse-eu-west-1/mnistclassifier-output"},
        "ResourceConfig": {
        ...
        "RoleArn": (
            "arn:aws:iam::861327392019:role/SageMakerFull"
            # "arn:aws:iam::AKIA4RCY47UJS6CSN4GE:role/service-role/"
    ...
    config={
        "Model": {
            ...
            "PrimaryContainer": {
                ...
                "ModelDataUrl": (
                    "s3://your-bucket-aawse-eu-west-1/mnistclassifier-output/mnistclassifier"
        ...
        "Endpoint": {
            ...
            # "EndpointName": "mnistclassifier-{{ execution_date.strftime('%Y-%m-%d-%H-%M-%S') }}",
        },
    },
    dag=dag,
)
